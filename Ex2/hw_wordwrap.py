...
def word_wrap(rect, font : pygame.font.Font, color, text :str):
    ...
    line = ''
    for word in words:
        if font.size(line + word + ' ')[0] <= rect.width:
            line += word + ' '
            continue
        if total_height + font.get_height() > rect.height:
            return surface
        text_surface = font.render(line, True, color)
        surface.blit(text_surface, (0, total_height))
